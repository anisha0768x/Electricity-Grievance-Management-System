    ...
    if conn:
        ...
        try:
            ...
            query = """
                SELECT 
                    i.issue_id, i.title, i.issue_type, i.description, i.status, i.priority,
                    i.created_at, i.resolved_at, i.image_url, d.division_name, d.division_id,
            ...
            query += " GROUP BY i.issue_id, i.title, i.issue_type, i.description, i.status, i.priority, i.created_at, i.resolved_at, i.image_url, d.division_name, d.division_id"
...
def worker_leave():
    ...
    try:
        # Get worker's leave applications - FIXED TABLE NAME
        cursor.execute("""
        ...
        flash(f'Error loading leave applications: {str(e)}', 'error')
        today = datetime.now().date()
        return render_template('worker_leave.html',
                           user_name=session['user_name'],
                           leave_applications=[],
...
def division_issues():
    ...
    try:
        ...
        query = """
            SELECT 
                i.issue_id, i.title, i.issue_type, i.description, i.status, i.priority,
                i.created_at, i.resolved_at, i.image_url,
                u.name as user_name,
                w.name as worker_name,
                iw.worker_status,
                iw.worker_notes,
        ...
        # REMOVE THIS IMAGE URL PROCESSING - use the URLs as they are in the database
        # for issue in issues:
        #     if issue['image_url']:
        #         # If image_url is just a filename, prepend the uploads path
        #         if not issue['image_url'].startswith(('http', '/', 'static/')):
        #             issue['image_url'] = f"/static/uploads/{issue['image_url']}"
...
def serve_uploaded_file(filename):
    ...
    # Adjust this path to where you're storing uploaded images
...
def raise_issue():
    """API endpoint to handle issue submission with image upload"""
    if not session.get('logged_in') or session.get('user_type') != 'user':
        return jsonify({'success': False, 'error': 'Not authorized'}), 401
    ...
    try:
        ...
        if 'issue_image' in request.files:
            image_file = request.files['issue_image']
            ...
            if image_file and image_file.filename != '':
                ...
                if ('.' in image_file.filename and 
                    ...
                    image_file.seek(0)  # Reset seek position
                    ...
                    import os
                    upload_folder = os.path.join(app.root_path, 'static', 'uploads', 'issues')
        ...
        print(f"DEBUG: Raising issue with user_id: {session.get('user_id')}, division_id: {division_id}, image_url: {image_url}")
        ...
        if conn:
            ...
            # Call the stored procedure with image_url
    ...
    if conn:
        ...
        try:
            cursor.execute("""
                SELECT 
                    i.issue_id, i.title, i.issue_type, i.description, i.status, i.priority,
                    i.created_at, i.resolved_at, i.latitude, i.longitude, i.image_url,  -- ← ADD image_url
    ...
    try:
        ...
        for key, value in request.args.items():
            if key not in ['log_type', 'date_from', 'date_to', 'search', 'limit', 'export'] and value:
                if log_type == 'security' and key in ['email', 'ip_address']:
                    query += f" AND f.{key} LIKE %s"
                    params.append(f"%{value}%")
                elif log_type == 'security' and key == 'user_type':
                    # Handle user_type filter for the calculated field
                    if value == 'Division Head':
                        query += " AND dh.head_id IS NOT NULL"
                    elif value == 'Worker':
                        query += " AND w.worker_id IS NOT NULL"
                    elif value == 'User':
                        query += " AND u.user_id IS NOT NULL"
                    elif value == 'Unknown':
                        query += " AND dh.head_id IS NULL AND w.worker_id IS NULL AND u.user_id IS NULL"
